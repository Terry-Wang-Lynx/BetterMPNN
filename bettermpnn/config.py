...
class DesignConfig:
    ...
    Format ``["A10", "A11", ...]`` = chain ID + 1-based position within that
    chain's sequence (not the PDB author number). Empty = redesign whole chain.
    """
    ...
    def __post_init__(self):
        for entry in self.redesign_residues:
            if not re.match(r'^[A-Za-z]\d+$', str(entry)):
                raise ValueError(f"Invalid redesign_residues entry {entry!r}; expected e.g. 'A10'.")
